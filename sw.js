...
self.onmessage = async event => {
    ...
    if (ES.payload !== undefined) {
        ...
        let diskRecord = await (await caches.open('S')).match(PTR);
        if (diskRecord) {
    ...
    if (C.payload && (!C.fn || ES.payload !== undefined)) {
        try {
            ...
            let prom = C.fn(event);
            if (prom?.catch) prom.catch(err => S.errors[S.errors[0]++ % 10000 + 1] = err.stack || String(err));
            
            ESS.haveidied = 0;

            if (ESS.persistence && ES.payload !== undefined) {
                await (await caches.open('S')).put(PTR, new Response(C.payload));
            }
            ESS.acknowledgement = true;
        } catch (error) {
            ES.last_error = S.errors[S.errors[0]++ % 10000 + 1] = error.stack || String(error);
            ESS.acknowledgement = false;
        }
    }
...
self.onfetch = event => {
    ...
    if (S[PTR] && S[PTR].fn) {
        try { 
            let prom = S[PTR].fn(event); 
            if (prom?.catch) prom.catch(err => S.errors[S.errors[0]++ % 10000 + 1] = err.stack || String(err));
        } catch (error) { 
            S.errors[S.errors[0]++ % 10000 + 1] = error.stack || String(error); 
        }
    } else {
        event.respondWith((async () => {
            ...
            if (!S.routeMap[event.clientId] && event.clientId) {
                ...
                if (activeClient) trueRoute = new URL(activeClient.url).pathname;
            }
            
            let diskRecord = await (await caches.open('S')).match(event.clientId) || await (await caches.open('S')).match(trueRoute);
            
            if (diskRecord) {
                let key = diskRecord.url.includes(event.clientId) ? event.clientId : trueRoute;
                let rawCode = await diskRecord.text();
                
                S[key] ??= {};
                S[key].status ??= { persistence: 1, multilinkfollow: key === event.clientId ? 1 : 0, clienturl: trueRoute };
                S[key].status.haveidied = 1;
                
                S[key].payload = rawCode;
                S[key].fn = new (async()=>{}).constructor('event', rawCode);
                ...
                try { 
                    let prom = S[key].fn(proxyEvent); 
                    if (prom?.catch) prom.catch(err => S.errors[S.errors[0]++ % 10000 + 1] = err.stack || String(err));
                    
                    S[key].status.haveidied = 0;
                } catch (error) { 
                    S.errors[S.errors[0]++ % 10000 + 1] = error.stack || String(error); 
                }
